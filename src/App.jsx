...
function nextStop(elevator, calls) {
  const assigned = calls.filter((call) => call.destination ? call.elevatorId === elevator.id : call.elevatorId === null || call.elevatorId === elevator.id)
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const pickupHere = calls.find((call) => !call.destination && elevators.some((elevator) => elevator.floor === call.pickup && (elevator.direction === 'Idle' || elevator.direction === call.travelDirection) && (call.elevatorId === null || call.elevatorId === elevator.id)))
    if (pickupHere) {
      const servicingElevator = elevators.find((elevator) => elevator.floor === pickupHere.pickup && (elevator.direction === 'Idle' || elevator.direction === pickupHere.travelDirection) && (pickupHere.elevatorId === null || pickupHere.elevatorId === elevator.id))
      setCalls((items) => items.map((call) => call.id === pickupHere.id ? { ...call, elevatorId: servicingElevator.id } : call))
      setWaitingCallId(pickupHere.id)
      return undefined
    }
    if (!calls.length) {
  ...
  function callElevator(pickup, travelDirection) {
    ...
    setCalls((items) => [...items, { id: `${pickup}-${travelDirection}-${Date.now()}`, pickup, travelDirection, destination: null, elevatorId: null }])
  }
  ...
  return <main className="app-shell">
    ...
    <section className="dashboard" aria-label="Elevator controls and status"><aside className="elevator-cards">{elevators.map((elevator) => <ElevatorCard key={elevator.id} elevator={elevator} calls={calls} speed={speed} setSpeed={setSpeed} />)}</aside><section className="floors" aria-label="Building floors"><div className="floor-heading"><h2>Call elevator</h2><p>Calls stay open until an elevator reaches them in the requested direction.</p></div><div className="floor-list">{FLOORS.map((item) => <div className={`floor-row ${elevators.some((elevator) => elevator.floor === item) ? 'current' : ''}`} key={item}><span>Floor <b>{item}</b></span><div className="call-buttons">{item < 30 && <button type="button" disabled={requestedCalls(item, 'Up')} onClick={() => callElevator(item, 'Up')}>↑ Up</button>}{item > 1 && <button type="button" disabled={requestedCalls(item, 'Down')} onClick={() => callElevator(item, 'Down')}>↓ Down</button>}</div></div>)}</div></section></section>
